...
from telegram.ext import (CallbackContext, CommandHandler, MessageHandler,
                          ...
                          ConversationHandler)
from database import (search_table_by_tg_id, insert_data, create_table,
                      delete_from_table, modify_idno)
...
def math_question(update: Update, context: CallbackContext) -> int:
  ...
  keyboard = [[
    InlineKeyboardButton(str(answers[0]),
                         callback_data="answer_" + str(answers[0])),
    InlineKeyboardButton(str(answers[1]),
                         callback_data="answer_" + str(answers[1])),
    InlineKeyboardButton(str(answers[2]),
                         callback_data="answer_" + str(answers[2]))
  ]]
  reply_markup = InlineKeyboardMarkup(keyboard)
...
def handle_math_answer(update: Update, context: CallbackContext) -> int:
  ...
  if user_answer == correct_answer:
    ...
    update.callback_query.answer(
      "Incorrect answer. Please try again. or /leave")
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.callback_query.message.reply_text(question,
                                             reply_markup=reply_markup)
    context.user_data['correct_answer'] = correct_answer
...
def get_password(update: Update, context: CallbackContext) -> int:
  ...
  if registered:
    ...
    update.message.bot.send_chat_action(chat_id=update.effective_chat.id,
                                        action=ChatAction.TYPING)

    profile = get_profile(campus=decrypt_data(reg_campus, KEY),
    ...
    if isinstance(profile, tuple):
      # Send the photo
      update.message.bot.send_photo(chat_id=update.effective_chat.id,
                                    photo=profile[0],
                                    caption=profile[1])
...
def view_profile(update: Update, context: CallbackContext) -> int:
  ...
  if registered:
    ...
    data = [
      ...
      [5, "Date of Registartion",
       decrypt_data(reg_date, KEY)],
    ]
...
def start(update: Update, context: CallbackContext) -> int:
  ...
  if registered:
    ...
    update.message.reply_photo(
      ...
      caption=
      "Welcome back %s!!\n  Click the button of your choice or use menu!"
      % decrypt_data(reg_name, KEY),
      reply_markup=ReplyKeyboardMarkup(
        LOGED_BUTTONS,
        resize_keyboard=True,
        one_time_keyboard=True,
...
def cancel(update: Update, context: CallbackContext) -> int:
  ...
  update.message.reply_text(
    f"Registration process has been canceled. You can start over by typing /start."
  )
  return ConversationHandler.END
...
def leave(update: Update, context: CallbackContext) -> int:
  ...
  update.message.reply_text(
    "Deletion process has been canceled. You can start over by typing /start.")
  return ConversationHandler.END
...
def registration(update: Update, context: CallbackContext) -> int:
  ...
  if query.data == "agree":
    ...
    context.bot.send_message(
      chat_id=chat_id,
      text=
      "Sorry, you must agree to the terms to use AAU Robot. Registration cannot proceed without agreement."
    )
    return ConversationHandler.END
...
def get_student_id(update: Update, context: CallbackContext) -> int:
  ...
  if is_user_id_valid(student_id):
    ...
    insert_data((str(tg_id), encrypt_data(student_id,
                                          KEY), encrypt_data(username, KEY),
                 encrypt_data(campus, KEY), encrypt_data(date_joined, KEY)))
    ...
    reply_markup = ReplyKeyboardMarkup(LOGED_BUTTONS,
                                       resize_keyboard=True,
                                       one_time_keyboard=True)
    update.message.reply_text("Please choose an option:",
                              reply_markup=reply_markup)
...
def filter_stickers(update: Update, context: CallbackContext) -> None:
  ...
  update.message.bot.send_chat_action(chat_id=update.effective_chat.id,
                                      action=ChatAction.TYPING)
  update.message.reply_text(
    f"Dear {user}, Currently, I don't search for Stickers!", quote=True)
...
def filter_animations(update: Update, context: CallbackContext) -> None:
  ...
  update.message.bot.send_chat_action(chat_id=update.effective_chat.id,
                                      action=ChatAction.TYPING)
  update.message.reply_text(
    f"Dear {user}, Currently, I don't search for animations!", quote=True)
...
def filter_attachments(update: Update, context: CallbackContext) -> None:
  ...
  update.message.bot.send_chat_action(chat_id=update.effective_chat.id,
                                      action=ChatAction.TYPING)
  update.message.reply_text(
    f"Dear {user}, Currently, I don't search for attachments!", quote=True)
...
def filter_audios(update: Update, context: CallbackContext) -> None:
  ...
  update.message.bot.send_chat_action(chat_id=update.effective_chat.id,
                                      action=ChatAction.TYPING)
  update.message.reply_text(
    f"Dear {user}, Currently, I don't search for Audios!", quote=True)
...
def filter_dice(update: Update, context: CallbackContext) -> None:
  ...
  update.message.bot.send_chat_action(chat_id=update.effective_chat.id,
                                      action=ChatAction.TYPING)
  update.message.reply_text(f"Dear {user}, Dice is beyound my knowlege!",
                            quote=True)
...
def main() -> None:
  ...
  conv_handler = ConversationHandler(
    ...
    states={
      AGREE:
      [CallbackQueryHandler(registration, pattern="^(agree|disagree)$")],
      CAMPUS:
      [CallbackQueryHandler(choose_campus, pattern="^(AAIT|AAU|EIABC)$")],
      STUDENT_ID:
  ...
  conv_handler_grade_report = ConversationHandler(
    entry_points=[
      MessageHandler(Filters.regex("^Grade Report$"), ask_for_password)
    ],
    states={
      GRADE_REPORT:
  ...
  conv_handler_account_deletion = ConversationHandler(
    entry_points=[
      MessageHandler(Filters.regex("^Delete Account$"), math_question)
    ],
    states={
      ACCOUNT_DELETED:
      [CallbackQueryHandler(handle_math_answer, pattern="^answer_")],
      ACCOUNT_NOT_DELETED: [
  ...
  dp.add_handler(MessageHandler(Filters.regex("^View Profile$"), view_profile))
